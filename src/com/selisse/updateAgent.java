...
//		doGet(request, response);
		...
		try{  
	            ...
	            for (FileItem ff : list) {
	            	if (ff.isFormField()) {
	            		...
	            		if(ff.getFieldName().equals("name")){
	            			...
	            			join_time=ds;
	            		}else if(ff.getFieldName().equals("agentID")){
	            			agentID=ds;
    		...
    		boolean isSuccess = common.updateAgent(name,type, mobile, balance, cert, agent_no,weixin_no,wangwang_no,join_time,img_path,agentID);
    		out.write((isSuccess ? "编辑成功了,棒棒哒 返回码:000000" : "编辑失败 返回码:999999"));
        }  
        }catch(Exception e){  
        	e.printStackTrace();  
