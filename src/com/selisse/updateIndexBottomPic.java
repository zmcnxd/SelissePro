 ...
 *
 */
public class updateIndexBottomPic extends HttpServlet {
	@Override
...
//		doGet(request, response);
		...
		try{  
	            ...
	            for (FileItem ff : list) {
	            	if (ff.isFormField()) {
	            		...
	            		if(ff.getFieldName().equals("pic_href")){
	            			pic_href=ds;
	            		}else if(ff.getFieldName().equals("indexPicID")){
	            			indexPicID=ds;
	            		}
            		}
	            	else{
	            		...
	            		if(!ff.getName().equals("")){
		            		...
		            		img_path = "/upload/indexBottomPics/" + dateStr + extend;
		            		//ss = ss.substring(ss.lastIndexOf("\\") + 1);//解析文件名
		            		String saveFileName = common.saveDir + "/indexBottomPics/" + dateStr + extend;
    		...
    		boolean isSuccess = common.updateIndexBottomPic(pic_href,img_path,indexPicID);
